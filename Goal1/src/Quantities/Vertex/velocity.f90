...
contains
    ...
    subroutine Calculate_derivatives(this, coordinates, total_vof, total_volume, nx, ny, nz, emf)
        ...
        !$omp parallel do simd collapse(3) num_threads(this%parallel_params%nthreads) default(private) shared(i, vof, vol, x, y, z, velocity_z, velocity_y, &
        !$omp& velocity_x, dvel_z_dz, dvel_z_dy, dvel_z_dx, dvel_y_dz, dvel_y_dy, dvel_y_dx, dvel_x_dz, &
        !$omp& dvel_x_dy, dvel_x_dx, emf, nx, ny, nz)
        do k = 1, nz
            do j = 1, ny
                do i = 1, nx
                    ip = i + 1
